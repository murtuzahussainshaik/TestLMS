    ...
    <div className="space-y-4">
      {lectures && lectures.length > 0 ? (
        lectures.map((lecture, index) => (
          <div
            key={lecture._id}
            className="border border-secondary-200 rounded-md overflow-hidden"
          >
            ...
            <div className="p-4 flex justify-between items-center">
              ...
              {isPurchased || isInstructor || lecture.isPreview ? (
                <Link
                  ...
                  className="ml-4 flex-shrink-0 flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary-100 text-primary-800"
                >
                  <PlayIcon className="h-4 w-4 mr-1" />
                  {lecture.isPreview ? "Preview" : "Play"}
                </Link>
              ) : (
                <div className="ml-4 flex-shrink-0 flex items-center px-3 py-1 rounded-full text-xs font-medium bg-secondary-100 text-secondary-800">
